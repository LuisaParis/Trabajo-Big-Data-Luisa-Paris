...
# Cargar mis docs
filenames <- list.files(getwd(), pattern = "Capitulo 4, Acuerdo 1.txt") 
...
write.csv(freq[ord],"word_freq_Capitulo4Acuerdo1.csv")

word_freq_acuerdos <- read.csv("word_freq_Capitulo4Acuerdo1.csv", header=TRUE,stringsAsFactors = T)
...
write.csv(ldaOut.topics,file=paste("LDAGibbs",k,"DocsToTopicsCap4Ac1.csv"))

#top 6 terms in each topic
ldaOut.terms <- as.matrix(terms(ldaOut,6))
write.csv(ldaOut.terms,file=paste("LDAGibbs",k,"TopicsToTermsCap4Ac1.csv"))

#probabilities associated with each topic assignment
topicProbabilities <- as.data.frame(ldaOut@gamma)
write.csv(topicProbabilities,file=paste("LDAGibbs",k,"TopicProbabilitiesCap4Ac1.csv"))
...
#write to file
write.csv(topic1ToTopic2,file=paste("LDAGibbs",k,"Topic1ToTopic2Cap4Ac1.csv"))
write.csv(topic2ToTopic3,file=paste("LDAGibbs",k,"Topic2ToTopic3Cap4Ac1.csv"))
...
beta_spread <- ap_topics %>%
  ...
  spread(topic, beta) %>%
  filter(topic3 > .001 | topic6 > .001) %>%
  mutate(log_ratio = log2(topic6 / topic3))
