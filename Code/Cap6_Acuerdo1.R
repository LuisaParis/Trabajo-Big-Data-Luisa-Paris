...
# Cargar mis docs
filenames <- list.files(getwd(), pattern = "Capitulo 6, Acuerdo 1.txt") 
...
myStopwords <- c("página", "pagina", "acuerdo", "final", "si", 
                 ...
                 "sala", "necesario", "ello", "deberá", "plane", "acceso", "bien", "buen", "enfoqu", "sosten")
...
write.csv(freq[ord],"word_freq_Capitulo6Acuerdo1.csv")

word_freq_acuerdos <- read.csv("word_freq_Capitulo6Acuerdo1.csv", header=TRUE,stringsAsFactors = T)
...
write.csv(ldaOut.topics,file=paste("LDAGibbs",k,"DocsToTopicsCap6Ac1.csv"))

#top 6 terms in each topic
ldaOut.terms <- as.matrix(terms(ldaOut,6))
write.csv(ldaOut.terms,file=paste("LDAGibbs",k,"TopicsToTermsCap6Ac1.csv"))

#probabilities associated with each topic assignment
topicProbabilities <- as.data.frame(ldaOut@gamma)
write.csv(topicProbabilities,file=paste("LDAGibbs",k,"TopicProbabilitiesCap6Ac1.csv"))
...
#write to file
write.csv(topic1ToTopic2,file=paste("LDAGibbs",k,"Topic1ToTopic2Cap6Ac1.csv"))
write.csv(topic2ToTopic3,file=paste("LDAGibbs",k,"Topic2ToTopic3Cap6Ac1.csv"))
